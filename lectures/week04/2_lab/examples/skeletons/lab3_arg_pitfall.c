 ...
 *
 * Buggy:   pthread_create(..., &i)        -- all threads share &i
 * Correct: pthread_create(..., &tids[i])  -- each thread gets its own copy
 *
 ...
 * have already incremented i. So multiple threads may print
...
void buggy_version(void)
{
    ...
    /*
     * TODO: Create threads using the BUGGY approach.
     *
     * Write a for loop (int i = 0; i < NUM_THREADS; i++):
     *   Call pthread_create(&threads[i], NULL, print_id, &i);
     *                                                    ^^
     *   Note: &i is the address of the loop variable!
     *   All threads share this same pointer.
     *
     * Then join all threads in a separate loop:
...
void correct_version(void)
{
    ...
    /*
     * TODO: Create threads using the CORRECT approach.
     *
     * Write a for loop (int i = 0; i < NUM_THREADS; i++):
     *   1. Set tids[i] = i;   (store the value in a separate array)
     *   2. Call pthread_create(&threads[i], NULL, print_id, &tids[i]);
     *                                                       ^^^^^^^
     *      Each thread gets a pointer to its own element.
     *
     * Then join all threads in a separate loop:
     *   for (int i = 0; i < NUM_THREADS; i++)
     *       pthread_join(threads[i], NULL);
