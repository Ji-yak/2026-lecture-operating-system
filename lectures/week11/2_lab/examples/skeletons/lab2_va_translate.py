...
def extract_fields(va):
    ...
    """
    # TODO: Extract the page offset from bits 11~0 of the virtual address
    # Hint: use bitwise AND with 0xFFF
    offset = 0

    # TODO: Extract VPN[0] from bits 20~12 of the virtual address
    # Hint: right-shift by 12, then AND with 0x1FF (9-bit mask)
    vpn0 = 0

    # TODO: Extract VPN[1] from bits 29~21 of the virtual address
    # Hint: right-shift by 21, then AND with 0x1FF
    vpn1 = 0

    # TODO: Extract VPN[2] from bits 38~30 of the virtual address
    # Hint: right-shift by 30, then AND with 0x1FF
    vpn2 = 0
...
def identify_region(va):
    ...
    """
    # TODO: Identify which xv6 memory region the virtual address belongs to.
    # Check the address against the following regions (in this order):
    #   - TRAMPOLINE (1 page at TRAMPOLINE address)
    #   - TRAPFRAME (1 page at TRAPFRAME address)
    #   - Kernel text/data (KERNBASE to PHYSTOP, with text below KERNBASE + 0x100000)
    #   - PLIC (starts at PLIC, size 0x4000000)
    #   - UART0 (1 page at UART0 address)
    #   - VIRTIO0 (1 page at VIRTIO0 address)
    #   - User process space (below 0x10000 for code/data/stack, below KERNBASE otherwise)
    # Return a descriptive string for the matching region.
...
def print_translation(va):
    ...
    # TODO: Print the 3-level page table translation process visualization.
    # Display the step-by-step walk:
    #   [Step 1] satp register -> L2 table base address
    #            Read PTE at L2 table[vpn2], extract PPN -> L1 table address
    #   [Step 2] Read PTE at L1 table[vpn1], extract PPN -> L0 table address
    #   [Step 3] Read PTE at L0 table[vpn0], extract PPN -> physical page base
    #   [Final]  Physical page base + offset = final physical address

    # TODO: Print the xv6 PX macro computation results.
    # Show: PX(2, va), PX(1, va), PX(0, va) with the shift amounts and results.
    # Recall: PX(level, va) = (va >> (PGSHIFT + 9*level)) & 0x1FF

    # TODO: For direct-mapped regions (KERNBASE~PHYSTOP, UART0, VIRTIO0, PLIC),
    # print a note that PA == VA.
