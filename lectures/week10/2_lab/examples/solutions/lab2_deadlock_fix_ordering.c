...
void *thread1_func(void *arg)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        /* pthread_mutex_lock(): Locks mutex_A first (first in the ordering rule).
         * If another thread already holds it, blocks until released. */
        pthread_mutex_lock(&mutex_A);
        printf("[Thread 1] Iteration %d: mutex_A locked\n", i + 1);

        /* usleep(): Short wait to induce race conditions.
         * Yields the CPU for 10ms to give Thread 2 a chance to run.
         * However, since Thread 2 also tries to acquire mutex_A first,
         * it proceeds in order without deadlock. */
        usleep(10000);  /* 10ms */

        printf("[Thread 1] Iteration %d: Attempting to lock mutex_B...\n", i + 1);
        /* pthread_mutex_lock(): Locks mutex_B second (follows ordering rule).
         * Since Thread 2 follows the same order, circular wait cannot occur. */
        pthread_mutex_lock(&mutex_B);
        printf("[Thread 1] Iteration %d: mutex_B locked\n", i + 1);
         ...
         * It is common practice to release in reverse order of acquisition (B -> A).
         * (LIFO order, like a stack) */
        pthread_mutex_unlock(&mutex_B);
        printf("[Thread 1] Iteration %d: mutex_B released\n", i + 1);
        pthread_mutex_unlock(&mutex_A);
        printf("[Thread 1] Iteration %d: mutex_A released\n", i + 1);
        printf("\n");
    }
...
void *thread2_func(void *arg)
{
    ...
    for (int i = 0; i < 5; i++) {
        ...
        /* pthread_mutex_lock(): Locks mutex_A first per the global ordering rule.
         * If Thread 1 already holds mutex_A, it waits here.
         * Thread 2 can only acquire mutex_A after Thread 1 finishes using
         * both mutex_B and releases both.
         * -> This makes it fundamentally impossible for both threads
         *    to hold each other's locks simultaneously. */
        pthread_mutex_lock(&mutex_A);
        printf("[Thread 2] Iteration %d: mutex_A locked\n", i + 1);

        usleep(10000);  /* 10ms wait - induce race condition */

        printf("[Thread 2] Iteration %d: Attempting to lock mutex_B...\n", i + 1);
        /* pthread_mutex_lock(): Locks mutex_B second (follows ordering rule) */
        pthread_mutex_lock(&mutex_B);
        printf("[Thread 2] Iteration %d: mutex_B locked\n", i + 1);

        /* Critical section: Perform work while holding both locks */
        printf("[Thread 2] Iteration %d: === Entered critical section (holding A and B) ===\n", i + 1);
        usleep(5000);  /* Simulate work for 5ms */

        /* Release mutexes: Release in reverse order of acquisition (B -> A) */
        pthread_mutex_unlock(&mutex_B);
        printf("[Thread 2] Iteration %d: mutex_B released\n", i + 1);
        pthread_mutex_unlock(&mutex_A);
        printf("[Thread 2] Iteration %d: mutex_A released\n", i + 1);
        printf("\n");
    }
