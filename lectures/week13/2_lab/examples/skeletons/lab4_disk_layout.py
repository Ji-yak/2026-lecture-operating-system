...
def calculate_layout():
    ...
    # TODO: Calculate the number of blocks for each region.
    #
    # 1. nlog: Log header (1 block) + LOGBLOCKS data blocks
    #    nlog = LOGBLOCKS + 1
    #
    # 2. ninodeblocks: Number of blocks needed for NINODES inodes
    #    ninodeblocks = NINODES // IPB + 1
    #    (integer division + 1 to handle remainder)
    #
    # 3. nbitmap: Number of bitmap blocks needed for FSSIZE blocks
    #    nbitmap = FSSIZE // BPB + 1
    #    (each bitmap block can track BPB blocks)
    #
    # 4. nmeta: Total metadata blocks = boot(1) + super(1) + log + inode + bitmap
    #
    # 5. nblocks: Data blocks = FSSIZE - nmeta
    nlog = 0
    ninodeblocks = 0
    nbitmap = 0
    nmeta = 0
    ...
    # TODO: Build the layout dictionary with start block and count for each region.
    # Each region follows sequentially on disk:
    #   boot:   starts at block 0, count = 1
    #   super:  starts at block 1, count = 1
    #   log:    starts at block 2, count = nlog
    #   inode:  starts at block 2 + nlog, count = ninodeblocks
    #   bitmap: starts at block 2 + nlog + ninodeblocks, count = nbitmap
    #   data:   starts at block nmeta, count = nblocks
    layout = {
        "boot":     {"start": 0, "count": 0},
        "super":    {"start": 0, "count": 0},
        "log":      {"start": 0, "count": 0},
        "inode":    {"start": 0, "count": 0},
        "bitmap":   {"start": 0, "count": 0},
...
def print_visual_layout(layout):
    ...
    # TODO: Create a proportional visualization bar of the disk layout.
    #
    # 1. Define the regions list with (name, layout_info, single_char) for each region:
    #    boot="B", super="S", log="L", inode="I", bitmap="M", data="D"
    #
    # 2. Set bar_width = 60 (total characters for the visualization)
    #
    # 3. For each region, calculate its width proportional to its block count:
    #    width = max(1, round(count / FSSIZE * bar_width))
    #    Build the bar string by repeating the region's character 'width' times
    #
    # 4. Adjust the bar to exactly bar_width characters
    #    (truncate if too long, pad with 'D' if too short)
    #
    # 5. Print the bar with block labels and a legend showing:
    #    character = region_name (Block start ~ end, count blocks, percentage)

    # TODO: Print a detailed ASCII art diagram showing the disk structure.
    # Show three rows:
    #   Row 1: boot | super | log (with block numbers)
    #   Row 2: inode blocks | bitmap (with block numbers)
    #   Row 3: data blocks (with block numbers)
    pass
...
def print_file_size_limits():
    ...
    # TODO: Calculate and print the current xv6 file size limit.
    #
    # Current xv6 uses direct + single indirect:
    #   maxfile = NDIRECT + NINDIRECT (total blocks)
    #   max_bytes = maxfile * BSIZE
    # Print the block counts and maximum file size in bytes and KB.

    # TODO: Calculate and print the file size limit with double indirect added.
    #
    # With double indirect:
    #   double_indirect = NINDIRECT * NINDIRECT (each indirect block points to NINDIRECT blocks)
    #   maxfile_di = NDIRECT + NINDIRECT + double_indirect
    #   max_bytes_di = maxfile_di * BSIZE
    # Print the block counts and maximum file size in bytes and MB.

    # TODO: Calculate and print the file size limit with triple indirect added.
    #
    ...
    # Print the block counts and maximum file size in bytes and GB.
    pass
...
def print_log_structure():
    ...
    # TODO: Calculate and print the maximum bytes filewrite() can write at once.
    #
    # Formula: max_write = ((MAXOPBLOCKS - 1 - 1 - 2) // 2) * BSIZE
    # The subtracted values account for:
    #   -1 for inode block, -1 for indirect block, -2 for bitmap + log header
    # Print the formula, substituted values, and result.
    pass
